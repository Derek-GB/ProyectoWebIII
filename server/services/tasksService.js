...
export const getAll = async () => {
  const [rows] = await pool.execute('SELECT * FROM tasks ORDER BY id DESC');  // Nota: tabla 'tasks' si traduciste
  return rows;
};

export const getByCompleted = async (completed) => {
  const [rows] = await pool.execute('SELECT * FROM tasks WHERE completed = ? ORDER BY id DESC', [completed ? 1 : 0]);
  return rows;
};

export const create = async (task, user) => {
